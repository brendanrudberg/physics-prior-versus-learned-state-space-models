...
build_RC_2c_G <- function(training) {
  ...
  model$addSystem(dTwo ~ ((Twi - Two)/(R2*(Ctot-f*Ctot)) + (Tao - Two)/(R3*(Ctot-f*Ctot)) + (Aw*G)/(Ctot-f*Ctot)) * dt + exp(p2)*dw2)
  ...
  model$setParameter(
    ...
    f   = c(initial = 0.5,  lower = 1e-3, upper = 0.999), #ratio between different wall section heat capacities
