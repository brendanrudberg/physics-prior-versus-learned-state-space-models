...
build_RC_3c_G <- function(training) {
  ...
  model$addSystem(dTwm ~ ((Twi - Twm)/(R2*(f2*(1-f1)*Ctot)) + (Two - Twm)/(R3*(f2*(1-f1)*Ctot))) * dt + exp(p2)*dw2)

  #Outer wall temperature ODE
  model$addSystem(dTwo ~ ((Twm - Two)/(R3*((1-f2)*(1-f1)*Ctot)) + (Tao - Two)/(R4*((1-f2)*(1-f1)*Ctot)) + (Aw*G)/((1-f2)*(1-f1)*Ctot)) * dt + exp(p3)*dw3)
  ...
  model$setParameter(
    ...
    p2  = c(initial = -1, lower = -30, upper = 10), # process noise for middle wall
    p3  = c(initial = -1, lower = -30, upper = 10), # process noise for outer wall
